...
import com.alphadraxonis.sandboxpixeldungeon.services.news.News;
...
import com.alphadraxonis.sandboxpixeldungeon.ui.OptionSlider;
			...
			if (DeviceCompat.isAndroid() && PixelScene.maxScreenZoom >= 2) {
				chkSaver = new CheckBox(Messages.get(this, "saver")) {
					...
					protected void onClick() {
						...
						if (checked()) {
							checked(!checked());
							SandboxPixelDungeon.scene().add(new WndOptions(Icons.get(Icons.DISPLAY),
		...
		protected void createChildren(Object... params) {
			...
			if (SPDSettings.interfaceSize() == 0) {
				btnToolbarSettings = new RedButton(Messages.get(this, "toolbar_settings"), 9){
					@Override
					protected void onClick() {
						SandboxPixelDungeon.scene().addToFront(new Window(){
		...
		protected void createChildren(Object... params) {
			...
			if (DeviceCompat.hasHardKeyboard()){
				...
				btnKeyBindings = new RedButton(Messages.get(this, "key_bindings")){
					...
					protected void onClick() {
						super.onClick();
						SandboxPixelDungeon.scene().addToFront(new WndKeyBindings(false));
					}
				};
			...
			if (ControllerHandler.isControllerConnected()){
				btnControllerBindings = new RedButton(Messages.get(this, "controller_bindings")){
					...
					protected void onClick() {
						super.onClick();
						SandboxPixelDungeon.scene().addToFront(new WndKeyBindings(true));
					}
				};
			...
			if (currLang != Languages.ENGLISH) {
				...
				btnCredits = new RedButton(credText, credText.length() > 9 ? 6 : 9) {
					...
					protected void onClick() {
						...
						SandboxPixelDungeon.scene().addToFront(credits);
					}
				};
				add(btnCredits);
			}
