		...
		if (Rankings.INSTANCE.latestDailyReplay != null){
			IconButton replayInfo = new IconButton(Icons.get(Icons.CALENDAR)){
				...
				protected void onClick() {
					SandboxPixelDungeon.scene().addToFront(new WndRanking(Rankings.INSTANCE.latestDailyReplay));
				}
		...
		for (long l : dates) {
			...
			if (first){
				IconButton latestInfo = new IconButton(Icons.INFO.get()){
					@Override
					protected void onClick() {
						SandboxPixelDungeon.scene().addToFront(new WndRanking(Rankings.INSTANCE.latestDaily));
					}
				};
