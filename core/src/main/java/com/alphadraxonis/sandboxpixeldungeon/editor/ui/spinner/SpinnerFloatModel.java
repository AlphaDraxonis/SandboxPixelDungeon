...
public class SpinnerFloatModel extends SpinnerIntegerModel {
    ...
    private static final char NUMBER_DECIMAL_SEPARATOR = Float.toString(1.1f).charAt(1);
    ...
    public void displayInputAnyNumberDialog(float min, float max) {
        ...
        w.getTextBox().convertStringToValidString = s -> {
            try {
                ...
                if (val < min) return Float.toString(min);
                if (val > max) return Float.toString(max);
                return s;
            } catch (NumberFormatException ex) {
