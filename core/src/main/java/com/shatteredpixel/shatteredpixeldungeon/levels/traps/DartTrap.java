...
public abstract class DartTrap extends Trap {
	...
	protected int dartDamage(Char target) {
		return Random.NormalIntRange(4, 8) - target.drRoll();
	}
	
	protected abstract void onHitEffect(Char target);
			...
			protected boolean act() {
				...
				if (target != null) {
					...
					if (Dungeon.level.heroFOV[pos] || Dungeon.level.heroFOV[target.pos]) {
						MissileSprite.missileFromPos(new PoisonDart(), pos, target.pos, new Callback() {
							@Override
							public void call() {
								int dmg = dartDamage(finalTarget);
								finalTarget.damage(dmg, DartTrap.this);
								if (finalTarget == Dungeon.hero && !finalTarget.isAlive()){
									Dungeon.fail( DartTrap.this  );
									GLog.n(Messages.get(DartTrap.this, "ondeath"));
									if (reclaimed) Badges.validateDeathFromFriendlyMagic();
								}
								onHitEffect(finalTarget);
						...
						finalTarget.damage(dartDamage(finalTarget), DartTrap.this);
						onHitEffect(finalTarget);
						return true;
					}
				} else {
