...
public class RageTrap extends Trap {
	...
	public void activate() {
		...
		for (Mob mob : Dungeon.level.mobs.toArray(new Mob[0])) {
			if (affectsChar(mob, x, y)) {
				Buff.prolong(mob, Amok.class, 20f);
			}
			mob.beckon( pos );
		}

		if (affectsChar(Dungeon.hero, x, y)) {
			Buff.prolong(Dungeon.hero, Vertigo.class, 20f);
		}


		if (Dungeon.level.heroFOV[pos]) {
			GLog.w( Messages.get(this, "alarm") );
			CellEmitter.center( pos ).start( Speck.factory( Speck.SCREAM ), 0.2f, 7 );
		}
