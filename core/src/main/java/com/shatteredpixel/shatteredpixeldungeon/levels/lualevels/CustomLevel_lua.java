...
public class CustomLevel_lua extends CustomLevel implements LuaLevel {
    ...
    public ItemsWithChanceDistrComp.RandomItemData getMobRotationVar() {
        ...
        if (luaScript != null && !luaScript.get("getMobRotationVar").isnil()) {
            try {
                ItemsWithChanceDistrComp.RandomItemData ret = (ItemsWithChanceDistrComp.RandomItemData) luaScript.get("getMobRotationVar").call(CoerceJavaToLua.coerce(this), vars).touserdata();
                return ret;
            } catch (LuaError error) { Game.runOnRenderThread(() -> DungeonScene.show(new WndError(error))); }
        }
    ...
    public ItemsWithChanceDistrComp.RandomItemData super_getMobRotationVar() {
        return super.getMobRotationVar();
    }
    ...
    public List getItems() {
        ...
        if (luaScript != null && !luaScript.get("getItems").isnil()) {
            try {
                List ret = (List) luaScript.get("getItems").call(CoerceJavaToLua.coerce(this), vars).touserdata();
                return ret;
            } catch (LuaError error) { Game.runOnRenderThread(() -> DungeonScene.show(new WndError(error))); }
        }
    ...
    protected int randomDropCell() {
        ...
        if (luaScript != null && !luaScript.get("randomDropCell").isnil()) {
            try {
                int ret = luaScript.get("randomDropCell").call(CoerceJavaToLua.coerce(this), vars).toint();
                return ret;
            } catch (LuaError error) { Game.runOnRenderThread(() -> DungeonScene.show(new WndError(error))); }
        }
    ...
    public int super_randomDropCell() {
        return super.randomDropCell();
    }
