...
public class MiningLevel_lua extends MiningLevel implements LuaLevel {
    ...
    public int questType() {
        LuaValue luaScript = levelScheme.luaScript.getScript();
        if (luaScript != null && !luaScript.get("questType").isnil()) {
            try {
                MethodOverride.A0<Integer> superMethod = () -> super.questType();
               int ret = luaScript.get("questType").call(CoerceJavaToLua.coerce(this), vars, CoerceJavaToLua.coerce(superMethod)).toint();
                return ret;
            } catch (LuaError error) { Game.runOnRenderThread(()->	DungeonScene.show(new WndError(error))); }
        }
    ...
    public void loadQuest() {
        LuaValue luaScript = levelScheme.luaScript.getScript();
        if (luaScript != null && !luaScript.get("loadQuest").isnil()) {
            try {
                MethodOverride.VoidA0 superMethod = () -> super.loadQuest();
               luaScript.get("loadQuest").call(CoerceJavaToLua.coerce(this), vars, CoerceJavaToLua.coerce(superMethod));
            } catch (LuaError error) { Game.runOnRenderThread(()->	DungeonScene.show(new WndError(error))); }
        }
        super.loadQuest();
    }
