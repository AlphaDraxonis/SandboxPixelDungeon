...
public class PrisonBossLevel_lua extends PrisonBossLevel implements LuaLevel {
    ...
    public PrisonBossLevel.State state() {
        ...
        if (luaScript != null && !luaScript.get("state").isnil()) {
            try {
                MethodOverride.A0<PrisonBossLevel.State> superMethod = () -> super.state();
               PrisonBossLevel.State ret = (PrisonBossLevel.State) luaScript.get("state").call(CoerceJavaToLua.coerce(this), vars, CoerceJavaToLua.coerce(superMethod)).touserdata();
                return ret;
            } catch (LuaError error) { Game.runOnRenderThread(() -> DungeonScene.show(new WndError(error))); }
        }
        return super.state();
    ...
    public void placeTrapsInTenguCell(float arg0) {
        ...
        if (luaScript != null && !luaScript.get("placeTrapsInTenguCell").isnil()) {
            try {
                MethodOverride.VoidA1 superMethod = (a0) -> super.placeTrapsInTenguCell((float) a0);
               luaScript.get("placeTrapsInTenguCell").invoke(new LuaValue[]{CoerceJavaToLua.coerce(this), vars, CoerceJavaToLua.coerce(superMethod), LuaValue.valueOf(arg0)}).arg1();
            } catch (LuaError error) { Game.runOnRenderThread(() -> DungeonScene.show(new WndError(error))); }
        }
    ...
    public int randomTenguCellPos() {
        ...
        if (luaScript != null && !luaScript.get("randomTenguCellPos").isnil()) {
            try {
                MethodOverride.A0<Integer> superMethod = () -> super.randomTenguCellPos();
               int ret = luaScript.get("randomTenguCellPos").call(CoerceJavaToLua.coerce(this), vars, CoerceJavaToLua.coerce(superMethod)).toint();
                return ret;
            } catch (LuaError error) { Game.runOnRenderThread(() -> DungeonScene.show(new WndError(error))); }
        }
    ...
    public void cleanTenguCell() {
        ...
        if (luaScript != null && !luaScript.get("cleanTenguCell").isnil()) {
            try {
                MethodOverride.VoidA0 superMethod = () -> super.cleanTenguCell();
               luaScript.get("cleanTenguCell").call(CoerceJavaToLua.coerce(this), vars, CoerceJavaToLua.coerce(superMethod));
            } catch (LuaError error) { Game.runOnRenderThread(() -> DungeonScene.show(new WndError(error))); }
        }
    ...
    public void progress() {
        ...
        if (luaScript != null && !luaScript.get("progress").isnil()) {
            try {
                MethodOverride.VoidA0 superMethod = () -> super.progress();
               luaScript.get("progress").call(CoerceJavaToLua.coerce(this), vars, CoerceJavaToLua.coerce(superMethod));
            } catch (LuaError error) { Game.runOnRenderThread(() -> DungeonScene.show(new WndError(error))); }
        }
        super.progress();
    }
