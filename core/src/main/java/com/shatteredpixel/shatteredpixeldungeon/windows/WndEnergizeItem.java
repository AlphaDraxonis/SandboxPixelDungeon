...
public class WndEnergizeItem extends WndInfoItem {
	...
	private static void energize(Item item){
		...
		if (SandboxPixelDungeon.scene() instanceof AlchemyScene){
			...
			((AlchemyScene) SandboxPixelDungeon.scene()).createEnergy();
			if (!item.isIdentified()){
				((AlchemyScene) SandboxPixelDungeon.scene()).showIdentify(item);
			}
	...
	public static WndBag openItemSelector(){
		if (SandboxPixelDungeon.scene() instanceof GameScene) {
			...
			SandboxPixelDungeon.scene().addToFront(window);
			return window;
		}
	}
		...
		public void onSelect(Item item) {
			if (item != null) {
				...
				if (SandboxPixelDungeon.scene() instanceof GameScene) {
					...
					SandboxPixelDungeon.scene().addToFront(new WndEnergizeItem(item, parentWnd));
				}
			}
		}
	};
