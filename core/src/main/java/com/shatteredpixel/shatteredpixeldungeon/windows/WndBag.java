...
public class WndBag extends WndTabbed {
	...
	protected void placeTitle( Bag bag, int width ){
        ...
        float titleWidth = addGoldAndEnergy(width);
    ...
    protected float addGoldAndEnergy(int width) {
        float titleWidth;
        if (Dungeon.energy == 0) {
        ...
        return new InventorySlot(item) {
			...
			protected boolean onLongClick() {
				if (selector == null && item.defaultAction() != null) {
					...
					DungeonScene.show(new WndInfoItem(item));
					return true;
				} else {
					return false;
				}
    ...
    public interface ItemSelectorInterface {
		...
		default boolean includeAddButtons() {
			return true;
		}
