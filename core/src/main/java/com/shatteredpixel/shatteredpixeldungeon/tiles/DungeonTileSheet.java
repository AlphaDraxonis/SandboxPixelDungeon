...
public class DungeonTileSheet {
	...
	public static HashSet<Integer> waterStitcheable = new HashSet<>(Arrays.asList(
			...
			Terrain.DOOR, Terrain.OPEN_DOOR, Terrain.MIMIC_DOOR, Terrain.COIN_DOOR, Terrain.HERO_LKD_DR, Terrain.LOCKED_DOOR, Terrain.CRYSTAL_DOOR
	));
	...
	private static int[] doorTiles = new int[]{
			Terrain.DOOR, Terrain.LOCKED_DOOR, Terrain.MIMIC_DOOR, Terrain.COIN_DOOR, Terrain.HERO_LKD_DR, Terrain.CRYSTAL_DOOR, Terrain.OPEN_DOOR
	};
	...
	public static int stitchInternalWallTile(int tile, int right, int rightBelow, int below, int leftBelow, int left){
		...
		else if (Dungeon.curLvlScheme().getVisualRegion() == LevelScheme.REGION_CAVES && tile == Terrain.WALL_DECO) result = WALL_INTERNAL_DECO;
	...
	public static int getVisualWithAlts(int visual, int pos){
		if (pos < 0) {
			...
			if (random >= 0.95f && rareAltVisuals.containsKey(visual))
				return rareAltVisuals.get(visual);
			else if (random >= 0.5f && commonAltVisuals.containsKey(visual))
