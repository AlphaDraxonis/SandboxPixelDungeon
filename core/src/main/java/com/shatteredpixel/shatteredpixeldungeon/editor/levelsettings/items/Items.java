...
public enum Items {
        ...
        switch (this) {
            ...
            case ALCHEMICAL:
                for (Class<?> cl : classes()) {
                    if (cl.isInstance(item)) return true;
                }
                return false;
            case OTHER:
    ...
    public List<Floor.ItemWithPos> filterStackSortItems(List<Floor.ItemWithPos> items) {
    ...
    public List<Floor.ItemWithPos> filterItems(List<Floor.ItemWithPos> items) {
        List<Floor.ItemWithPos> ret = new ArrayList<>();
        for (Floor.ItemWithPos i : items) {
            if (isInCategory(i.item())) ret.add(i);
        }
        return ret;
    }

    public List<Floor.ItemWithPos> stackItems(List<Floor.ItemWithPos> items) {
        List<Floor.ItemWithPos> ret = new LinkedList<>();
        Map<Class<?>, Set<Floor.ItemWithPos>> stackableItems = new HashMap<>();
        for (Floor.ItemWithPos item : items) {
            ...
            if (!i.isUpgradable()&&!(i instanceof  SpiritBow)) {
                ...
                if (stackableItems.containsKey(cl)) {
                    for (Floor.ItemWithPos st : stackableItems.get(cl)) {
                ...
                if (!added) {
                    item = new Floor.ItemWithPos(Reflection.newInstance(cl), item.pos());
                    Set<Floor.ItemWithPos> set = stackableItems.get(cl);
    ...
    public List<Floor.ItemWithPos> sortItems(List<Floor.ItemWithPos> unsortedItems) {
        List<Floor.ItemWithPos> sortedItems = new ArrayList<>();
        List<Floor.ItemWithPos> toSort = new ArrayList<>(unsortedItems);

        for (Class<?> clazz : classes()) {
            for (Floor.ItemWithPos item : unsortedItems) {
