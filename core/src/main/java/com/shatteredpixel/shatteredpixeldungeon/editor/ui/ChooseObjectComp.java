...
public abstract class ChooseObjectComp extends Component {
    ...
    protected void layout() {
        ...
        if (displayWidth == -1) {
            label.maxWidth((int) ((changeObjBtn.left() - x - 1)*2/3f));
            displayWidth = (changeObjBtn.left() - x - label.width() - 3 - 2);
        }
        display.setRect(changeObjBtn.left() - 2 - displayWidth, y, displayWidth, height);

        label.maxWidth((int) (display.left() - 2 - x));
