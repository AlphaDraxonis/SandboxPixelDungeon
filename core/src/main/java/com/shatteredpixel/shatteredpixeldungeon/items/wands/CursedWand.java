...
//				Game.switchScene(InterlevelScene.class);
	...
	public static class FireBall extends CursedEffect {
		...
		public boolean effect(Item origin, Char user, Ballistica bolt, boolean positiveOnly) {
			...
			ShadowCaster.castShadow(c.x, c.y, Dungeon.level.width(), fieldOfView, Dungeon.level.solid, 3, false);
	...
	public static class ConeOfColors extends CursedEffect {
		...
		public void FX(Item origin, Char user, Ballistica bolt, Callback callback) {
			...
			bolt = new Ballistica(bolt.sourcePos, bolt.collisionPos, Ballistica.STOP_SOLID, null);
		...
		public boolean effect(Item origin, Char user, Ballistica bolt, boolean positiveOnly) {
			...
			try {
				...
				if(Messages.lang() != Languages.ENGLISH){
					...
					SandboxPixelDungeon.runOnRenderThread(
							new Callback() {
								@Override
								public void call() {
									GameScene.show(
	...
	public static class SinkHole extends CursedEffect {
		...
		public boolean effect(Item origin, Char user, Ballistica bolt, boolean positiveOnly) {
			...
			for (int i = 0; i < PathFinder.distance.length; i++) {
				if (PathFinder.distance[i] < Integer.MAX_VALUE) {
					if (!Dungeon.level.solid[i] || Dungeon.level.isPassable(i)) {
