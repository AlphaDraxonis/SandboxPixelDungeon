...
abstract public class MissileWeapon extends Weapon {
	...
	public boolean isSimilar( Item item ) {
		return trueLevel() == item.trueLevel()
				...
				&& baseUses == ((MissileWeapon) item).baseUses;
	}
	...
	public int throwPos(Hero user, int dst) {
		...
		if (projecting > 0
				&& (Dungeon.level.isPassable(dst) || Dungeon.level.avoid[dst] || Actor.findChar(dst) != null)
	...
	public int proc(Char attacker, Char defender, int damage) {
		if (attacker instanceof Hero && Random.Int(3) < ((Hero) attacker).pointsInTalent(Talent.SHARED_ENCHANTMENT)){
			...
			if (bow != null && bow.enchantment != null && ((Hero) attacker).buff(MagicImmune.class) == null) {
