...
public class ArcaneBomb extends Bomb {
	...
	public static class ArcaneBombFuse extends Fuse {
		...
		private final ArrayList<Emitter> gooWarnEmitters = new ArrayList<>();
		...
		public Fuse ignite(Bomb bomb) {
			...
			Actor.add(new Actor() {
				...
				protected boolean act() {
					...
					if (bombPos != -1) {
						PathFinder.buildDistanceMapForEnvironmentals(bombPos, BArray.not(Dungeon.level.solid, null), bomb.explosionRange());
						for (int i = 0; i < PathFinder.distance.length; i++) {
							if (PathFinder.distance[i] < Integer.MAX_VALUE) {
