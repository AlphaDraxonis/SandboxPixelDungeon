...
public class Crossbow extends MeleeWeapon {
	...
	public int proc(Char attacker, Char defender, int damage) {
		...
		if (attacker.buff(ChargedShot.class) != null && !(curItem instanceof Dart)){
			...
			Ballistica trajectory = new Ballistica(attacker.pos, defender.pos, Ballistica.STOP_TARGET, null);
			//trim it to just be the part that goes past them
			trajectory = new Ballistica(trajectory.collisionPos, trajectory.path.get(trajectory.path.size()-1), Ballistica.REAL_PROJECTILE, null);
