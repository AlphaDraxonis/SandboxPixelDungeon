...
public interface LuaCustomObjectClass extends CustomObjectClass {
	...
	default boolean isOriginal() {
		return UserContentManager.getLuaClass(getIdentifier()) == this;
	}
	...
	default void onRestoreFromBundle(Bundle bundle) {
		...
		LuaValue script;
		if (!CustomDungeon.isEditing() && (script = UserContentManager.getScript(getIdentifier())) != null && script.get("vars").istable()) {
