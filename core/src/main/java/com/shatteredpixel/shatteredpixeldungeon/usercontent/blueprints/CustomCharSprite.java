...
import com.shatteredpixel.shatteredpixeldungeon.usercontent.interfaces.LuaCharSprite;
import com.shatteredpixel.shatteredpixeldungeon.usercontent.interfaces.LuaClassGenerator;
import com.shatteredpixel.shatteredpixeldungeon.usercontent.ui.editcomps.CustomCharSpriteEditor;
...
public class CustomCharSprite extends LuaCustomObject {
	...
	//TODO tzz if a mob uses a byte buddy char sprite instance, changing the sprite via normal menu isn't possible

	//if without luaScript: use default sprite class but change the texture!
	private Class<? extends Mob> targetClass; // can only be used if the CustomMob is this class; must be set when importing and can only be changed by tzz??idk
	...
	public void reloadSprite() {
		super.reloadSprite();
		for (Mob mob : UserContentManager.getAllCustomObjects(Mob.class)) {
			int ident = ((CustomMobClass) mob).getIdentifier();
			CustomMob customMob = UserContentManager.getUserContent(ident, CustomMob.class);
