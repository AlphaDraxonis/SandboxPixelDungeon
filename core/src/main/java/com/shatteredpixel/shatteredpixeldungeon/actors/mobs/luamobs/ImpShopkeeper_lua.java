...
public class ImpShopkeeper_lua extends ImpShopkeeper implements LuaMob {
...
@Override
    ...
    public String chatText() {
        LuaValue luaScript = CustomObject.getScript(identifier);
        if (luaScript != null && !luaScript.get("chatText").isnil()) {
            try {
                MethodOverride.A0<String> superMethod = () -> super.chatText();
               String ret = luaScript.get("chatText").call(CoerceJavaToLua.coerce(this), vars, CoerceJavaToLua.coerce(superMethod)).tojstring();
                return ret;
            } catch (LuaError error) { Game.runOnRenderThread(()->	DungeonScene.show(new WndError(error))); }
        }
        return super.chatText();
    }
    ...
    public void processHarm() {
        LuaValue luaScript = CustomObject.getScript(identifier);
        if (luaScript != null && !luaScript.get("processHarm").isnil()) {
            try {
                MethodOverride.VoidA0 superMethod = () -> super.processHarm();
               luaScript.get("processHarm").call(CoerceJavaToLua.coerce(this), vars, CoerceJavaToLua.coerce(superMethod));
            } catch (LuaError error) { Game.runOnRenderThread(()->	DungeonScene.show(new WndError(error))); }
        }
        super.processHarm();
    }
