...
public class Hero extends Char {
	...
	public static int heroDamageIntRange(int min, int max ){
		if (Random.Float() < ThirteenLeafClover.alterHeroDamageChance(Dungeon.hero)){
	...
	private boolean actOpenChest( HeroAction.OpenChest action ) {
		...
		if (Dungeon.level.adjacent( pos, dst ) || pos == dst) {
			...
			if (heap != null && (heap.type != Type.HEAP && heap.type != Type.FOR_SALE)) {
				...
				if ((heap.type == Type.LOCKED_CHEST && Notes.keyCount(new GoldenKey(Dungeon.levelName, dst)) < 1)
					|| (heap.type == Type.CRYSTAL_CHEST && Notes.keyCount(new CrystalKey(Dungeon.levelName, dst)) < 1)){
			...
			if ((Dungeon.level.map[action.dst] == Terrain.WALL
				...
				sprite.attack(action.dst, new Callback() {
					...
					public void call() {
						...
						} else if (Dungeon.level.map[action.dst] == Terrain.WALL || Dungeon.level.map[action.dst] == Terrain.WALL_DECO){
	...
	public void rest( boolean fullRest ) {
		...
		if (hasTalent(Talent.PATIENT_STRIKE)){
			Buff.affect(this, Talent.PatientStrikeTracker.class).pos = Hero.this.pos;
		}
		if (!fullRest) {
			if (sprite != null) {
	...
	public void checkVisibleMobs() {
		...
		for (Mob m : Dungeon.level.mobs.toArray(new Mob[0])) {
			if (this == Dungeon.hero && fieldOfView[ m.pos ] && m.landmark() != null){
	...
	public void die( Object cause ) {
		...
		if (Dungeon.reachedCheckpoint != null) {
			...
			InterlevelScene.mode = InterlevelScene.Mode.CHECKPOINT;
			SandboxPixelDungeon.switchScene(InterlevelScene.class);
			return;
		}
	...
	public void onAttackComplete() {
		...
		if (this == Dungeon.hero) AttackIndicator.target(attackTarget);
		boolean wasEnemy;
		if (alignment != Alignment.NEUTRAL) wasEnemy = alignment != attackTarget.alignment;
		else wasEnemy = false;
		if (!wasEnemy) wasEnemy = attackTarget instanceof Mimic && attackTarget.alignment == Alignment.NEUTRAL;
		...
		if (curAction instanceof HeroAction.Unlock) {
			...
			if (Dungeon.level.distance(pos, doorCell) <= 1) {
				...
				if (door == Terrain.LOCKED_DOOR) {
					...
					if (hasKey) {
						if (!Dungeon.customDungeon.permaKey) Dungeon.gold -= Dungeon.level.getCoinDoorCost(doorCell);
