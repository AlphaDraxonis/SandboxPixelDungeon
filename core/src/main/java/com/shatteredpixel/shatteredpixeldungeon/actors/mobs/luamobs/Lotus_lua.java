...
@Override
    ...
    public int getLvl() {
        ...
        if (luaScript != null && !luaScript.get("getLvl").isnil()) {
            try {
                int ret = luaScript.get("getLvl").call(CoerceJavaToLua.coerce(this), vars).toint();
                return ret;
            } catch (LuaError error) { Game.runOnRenderThread(() -> DungeonScene.show(new WndError(error))); }
        }
        return super.getLvl();
    ...
    public float seedPreservation() {
        ...
        if (luaScript != null && !luaScript.get("seedPreservation").isnil()) {
            try {
                float ret = luaScript.get("seedPreservation").call(CoerceJavaToLua.coerce(this), vars).tofloat();
                return ret;
            } catch (LuaError error) { Game.runOnRenderThread(() -> DungeonScene.show(new WndError(error))); }
        }
    ...
    public float super_seedPreservation() {
        return super.seedPreservation();
    }
    ...
    public boolean inRange(int arg0) {
        ...
        if (luaScript != null && !luaScript.get("inRange").isnil()) {
            try {
                boolean ret = luaScript.get("inRange").call(CoerceJavaToLua.coerce(this), vars, LuaValue.valueOf(arg0)).toboolean();
                return ret;
            } catch (LuaError error) { Game.runOnRenderThread(() -> DungeonScene.show(new WndError(error))); }
        }
    ...
    public boolean super_inRange(int arg0) {
        return super.inRange(arg0);
    }
