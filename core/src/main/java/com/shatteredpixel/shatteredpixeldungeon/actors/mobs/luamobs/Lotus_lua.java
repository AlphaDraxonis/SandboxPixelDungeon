...
public class Lotus_lua extends WandOfRegrowth.Lotus implements LuaMob {
...
@Override
    ...
    public int getLvl() {
        LuaValue luaScript = CustomObject.getScript(identifier);
        if (luaScript != null && !luaScript.get("getLvl").isnil()) {
            try {
                MethodOverride.A0<Integer> superMethod = () -> super.getLvl();
               int ret = luaScript.get("getLvl").call(CoerceJavaToLua.coerce(this), vars, CoerceJavaToLua.coerce(superMethod)).toint();
                return ret;
            } catch (LuaError error) { Game.runOnRenderThread(()->	DungeonScene.show(new WndError(error))); }
        }
        return super.getLvl();
    ...
    public float seedPreservation() {
        LuaValue luaScript = CustomObject.getScript(identifier);
        if (luaScript != null && !luaScript.get("seedPreservation").isnil()) {
            try {
                MethodOverride.A0<Float> superMethod = () -> super.seedPreservation();
               float ret = luaScript.get("seedPreservation").call(CoerceJavaToLua.coerce(this), vars, CoerceJavaToLua.coerce(superMethod)).tofloat();
                return ret;
            } catch (LuaError error) { Game.runOnRenderThread(()->	DungeonScene.show(new WndError(error))); }
        }
        return super.seedPreservation();
    }
    ...
    @Override
    public boolean inRange(int arg0) {
        LuaValue luaScript = CustomObject.getScript(identifier);
        if (luaScript != null && !luaScript.get("inRange").isnil()) {
            try {
                MethodOverride.A1<Boolean> superMethod = (a0) -> super.inRange((int) a0);
               boolean ret = luaScript.get("inRange").invoke(new LuaValue[]{CoerceJavaToLua.coerce(this), vars, CoerceJavaToLua.coerce(superMethod), LuaValue.valueOf(arg0)}).arg1().toboolean();
                return ret;
            } catch (LuaError error) { Game.runOnRenderThread(()->	DungeonScene.show(new WndError(error))); }
        }
        return super.inRange(arg0);
    }
