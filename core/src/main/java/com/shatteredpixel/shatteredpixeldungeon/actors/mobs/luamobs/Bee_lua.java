...
@Override
    ...
    public int potHolderID() {
        ...
        if (luaScript != null && !luaScript.get("potHolderID").isnil()) {
            try {
                int ret = luaScript.get("potHolderID").call(CoerceJavaToLua.coerce(this), vars).toint();
                return ret;
            } catch (LuaError error) { Game.runOnRenderThread(() -> DungeonScene.show(new WndError(error))); }
        }
    ...
    public int potPos() {
        ...
        if (luaScript != null && !luaScript.get("potPos").isnil()) {
            try {
                int ret = luaScript.get("potPos").call(CoerceJavaToLua.coerce(this), vars).toint();
                return ret;
            } catch (LuaError error) { Game.runOnRenderThread(() -> DungeonScene.show(new WndError(error))); }
        }
        return super.potPos();
    ...
    public void setPotInfo(int arg0, Char arg1) {
        ...
        if (luaScript != null && !luaScript.get("setPotInfo").isnil()) {
            try {
                luaScript.get("setPotInfo").invoke(new LuaValue[]{CoerceJavaToLua.coerce(this), vars, LuaValue.valueOf(arg0), CoerceJavaToLua.coerce(arg1)}).arg1();
                return;
            } catch (LuaError error) { Game.runOnRenderThread(() -> DungeonScene.show(new WndError(error))); }
        }
    ...
    public void super_setPotInfo(int arg0, Char arg1) {
        super.setPotInfo(arg0, arg1);
    }
