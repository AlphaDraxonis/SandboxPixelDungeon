...
public class Bee_lua extends Bee implements LuaMob {
...
@Override
    ...
    @Override
    public void setPotInfo(int arg0, Char arg1) {
        LuaValue luaScript = CustomObject.getScript(identifier);
        if (luaScript != null && !luaScript.get("setPotInfo").isnil()) {
            try {
                MethodOverride.VoidA2 superMethod = (a0, a1) -> super.setPotInfo((int) a0, (Char) a1);
               luaScript.get("setPotInfo").invoke(new LuaValue[]{CoerceJavaToLua.coerce(this), vars, CoerceJavaToLua.coerce(superMethod), LuaValue.valueOf(arg0), CoerceJavaToLua.coerce(arg1)}).arg1();
            } catch (LuaError error) { Game.runOnRenderThread(()->	DungeonScene.show(new WndError(error))); }
        }
        super.setPotInfo(arg0, arg1);
    }
    ...
    public int potHolderID() {
        LuaValue luaScript = CustomObject.getScript(identifier);
        if (luaScript != null && !luaScript.get("potHolderID").isnil()) {
            try {
                MethodOverride.A0<Integer> superMethod = () -> super.potHolderID();
               int ret = luaScript.get("potHolderID").call(CoerceJavaToLua.coerce(this), vars, CoerceJavaToLua.coerce(superMethod)).toint();
                return ret;
            } catch (LuaError error) { Game.runOnRenderThread(()->	DungeonScene.show(new WndError(error))); }
        }
        return super.potHolderID();
    }
    ...
    public int potPos() {
        LuaValue luaScript = CustomObject.getScript(identifier);
        if (luaScript != null && !luaScript.get("potPos").isnil()) {
            try {
                MethodOverride.A0<Integer> superMethod = () -> super.potPos();
               int ret = luaScript.get("potPos").call(CoerceJavaToLua.coerce(this), vars, CoerceJavaToLua.coerce(superMethod)).toint();
                return ret;
            } catch (LuaError error) { Game.runOnRenderThread(()->	DungeonScene.show(new WndError(error))); }
        }
        return super.potPos();
