...
//	}
	...
	protected boolean canAttack( Char enemy ) {
		return !Dungeon.level.adjacent( pos, enemy.pos )
				&& (super.canAttack(enemy) || new Ballistica( pos, enemy.pos, Ballistica.REAL_PROJECTILE, null).collisionPos == enemy.pos);
	}
	...
	public int attackProc( Char enemy, int damage ) {
		...
		if (effect > 2) {
			...
			if (effect >=6 && enemy.buff(Burning.class) == null){
				...
				if (Dungeon.level.isFlamable(enemy.pos))
	...
	protected class Wandering extends Mob.Wandering{
		...
		protected int randomDestination() {
			...
			PathFinder.buildDistanceMap(Dungeon.hero.pos, Dungeon.level.getPassableVar(GnollTrickster.this), GnollTrickster.this);
