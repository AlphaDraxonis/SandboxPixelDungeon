...
public class Sentry_lua extends SentryRoom.Sentry implements LuaMob {
...
@Override
    ...
    public float getInitialChargeDelay() {
        LuaValue luaScript = CustomObject.getScript(identifier);
        if (luaScript != null && !luaScript.get("getInitialChargeDelay").isnil()) {
            try {
                MethodOverride.A0<Float> superMethod = () -> super.getInitialChargeDelay();
               float ret = luaScript.get("getInitialChargeDelay").call(CoerceJavaToLua.coerce(this), vars, CoerceJavaToLua.coerce(superMethod)).tofloat();
                return ret;
            } catch (LuaError error) { Game.runOnRenderThread(()->	DungeonScene.show(new WndError(error))); }
        }
        return super.getInitialChargeDelay();
    }
    ...
    @Override
    public void setInitialChargeDelay(float arg0) {
        LuaValue luaScript = CustomObject.getScript(identifier);
        if (luaScript != null && !luaScript.get("setInitialChargeDelay").isnil()) {
            try {
                MethodOverride.VoidA1 superMethod = (a0) -> super.setInitialChargeDelay((float) a0);
               luaScript.get("setInitialChargeDelay").invoke(new LuaValue[]{CoerceJavaToLua.coerce(this), vars, CoerceJavaToLua.coerce(superMethod), LuaValue.valueOf(arg0)}).arg1();
            } catch (LuaError error) { Game.runOnRenderThread(()->	DungeonScene.show(new WndError(error))); }
        }
        super.setInitialChargeDelay(arg0);
    }
