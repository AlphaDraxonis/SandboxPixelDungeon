...
public class SpectralNecromancer_lua extends SpectralNecromancer implements LuaMob {
...
@Override
    ...
    @Override
    protected Mob convertToSummonedMob(Mob arg0) {
        LuaValue luaScript = CustomObject.getScript(identifier);
        if (luaScript != null && !luaScript.get("convertToSummonedMob").isnil()) {
            try {
                MethodOverride.A1<Mob> superMethod = (a0) -> super.convertToSummonedMob((Mob) a0);
               Mob ret = (Mob) luaScript.get("convertToSummonedMob").invoke(new LuaValue[]{CoerceJavaToLua.coerce(this), vars, CoerceJavaToLua.coerce(superMethod), CoerceJavaToLua.coerce(arg0)}).arg1().touserdata();
                return ret;
            } catch (LuaError error) { Game.runOnRenderThread(()->	DungeonScene.show(new WndError(error))); }
        }
        return super.convertToSummonedMob(arg0);
    }
    ...
    public Mob summonMinion() {
        LuaValue luaScript = CustomObject.getScript(identifier);
        if (luaScript != null && !luaScript.get("summonMinion").isnil()) {
            try {
                MethodOverride.A0<Mob> superMethod = () -> super.summonMinion();
               Mob ret = (Mob) luaScript.get("summonMinion").call(CoerceJavaToLua.coerce(this), vars, CoerceJavaToLua.coerce(superMethod)).touserdata();
                return ret;
            } catch (LuaError error) { Game.runOnRenderThread(()->	DungeonScene.show(new WndError(error))); }
        }
        return super.summonMinion();
    }
    ...
    protected Mob createSummonedMob() {
        LuaValue luaScript = CustomObject.getScript(identifier);
        if (luaScript != null && !luaScript.get("createSummonedMob").isnil()) {
            try {
                MethodOverride.A0<Mob> superMethod = () -> super.createSummonedMob();
               Mob ret = (Mob) luaScript.get("createSummonedMob").call(CoerceJavaToLua.coerce(this), vars, CoerceJavaToLua.coerce(superMethod)).touserdata();
                return ret;
            } catch (LuaError error) { Game.runOnRenderThread(()->	DungeonScene.show(new WndError(error))); }
        }
        return super.createSummonedMob();
    }
