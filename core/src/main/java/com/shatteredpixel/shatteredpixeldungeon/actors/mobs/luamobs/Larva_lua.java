...
public class Larva_lua extends YogDzewa.Larva implements LuaMob {
...
@Override
    ...
    public void setId(int arg0) {
        LuaValue luaScript = CustomObject.getScript(identifier);
        if (luaScript != null && !luaScript.get("setId").isnil()) {
            try {
                MethodOverride.VoidA1 superMethod = (a0) -> super.setId((int) a0);
               luaScript.get("setId").invoke(new LuaValue[]{CoerceJavaToLua.coerce(this), vars, CoerceJavaToLua.coerce(superMethod), LuaValue.valueOf(arg0)}).arg1();
            } catch (LuaError error) { Game.runOnRenderThread(() -> DungeonScene.show(new WndError(error))); }
        }
        super.setId(arg0);
    }
    ...
    public int getId() {
        LuaValue luaScript = CustomObject.getScript(identifier);
        if (luaScript != null && !luaScript.get("getId").isnil()) {
            try {
                MethodOverride.A0<Integer> superMethod = () -> super.getId();
               int ret = luaScript.get("getId").call(CoerceJavaToLua.coerce(this), vars, CoerceJavaToLua.coerce(superMethod)).toint();
                return ret;
            } catch (LuaError error) { Game.runOnRenderThread(() -> DungeonScene.show(new WndError(error))); }
        }
        return super.getId();
