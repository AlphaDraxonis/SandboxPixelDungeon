...
public class Bandit_lua extends Bandit implements LuaMob {
...
@Override
    ...
    @Override
    protected boolean steal(Hero arg0) {
        LuaValue luaScript = CustomObject.getScript(identifier);
        if (luaScript != null && !luaScript.get("steal").isnil()) {
            try {
                MethodOverride.A1<Boolean> superMethod = (a0) -> super.steal((Hero) a0);
               boolean ret = luaScript.get("steal").invoke(new LuaValue[]{CoerceJavaToLua.coerce(this), vars, CoerceJavaToLua.coerce(superMethod), CoerceJavaToLua.coerce(arg0)}).arg1().toboolean();
                return ret;
            } catch (LuaError error) { Game.runOnRenderThread(() -> DungeonScene.show(new WndError(error))); }
        }
        return super.steal(arg0);
    }
