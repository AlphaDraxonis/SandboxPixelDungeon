...
public class TormentedSpirit_lua extends TormentedSpirit implements LuaMob {
...
@Override
    ...
    public void cleanse() {
        LuaValue luaScript = CustomObject.getScript(identifier);
        if (luaScript != null && !luaScript.get("cleanse").isnil()) {
            try {
                luaScript.get("cleanse").call(CoerceJavaToLua.coerce(this), vars);
                return;
            } catch (LuaError error) { Game.runOnRenderThread(() -> DungeonScene.show(new WndError(error))); }
        }
        super.cleanse();
