...
public class YogDzewa_lua extends YogDzewa implements LuaMob {
...
@Override
    ...
    @Override
    public int addFist(YogFist arg0) {
        LuaValue luaScript = CustomObject.getScript(identifier);
        if (luaScript != null && !luaScript.get("addFist").isnil()) {
            try {
                MethodOverride.A1<Integer> superMethod = (a0) -> super.addFist((YogFist) a0);
               int ret = luaScript.get("addFist").invoke(new LuaValue[]{CoerceJavaToLua.coerce(this), vars, CoerceJavaToLua.coerce(superMethod), CoerceJavaToLua.coerce(arg0)}).arg1().toint();
                return ret;
            } catch (LuaError error) { Game.runOnRenderThread(() -> DungeonScene.show(new WndError(error))); }
        }
        return super.addFist(arg0);
    }
    ...
    @Override
    public void updateVisibility(Level arg0) {
        LuaValue luaScript = CustomObject.getScript(identifier);
        if (luaScript != null && !luaScript.get("updateVisibility").isnil()) {
            try {
                MethodOverride.VoidA1 superMethod = (a0) -> super.updateVisibility((Level) a0);
               luaScript.get("updateVisibility").invoke(new LuaValue[]{CoerceJavaToLua.coerce(this), vars, CoerceJavaToLua.coerce(superMethod), CoerceJavaToLua.coerce(arg0)}).arg1();
            } catch (LuaError error) { Game.runOnRenderThread(() -> DungeonScene.show(new WndError(error))); }
        }
        super.updateVisibility(arg0);
    }
