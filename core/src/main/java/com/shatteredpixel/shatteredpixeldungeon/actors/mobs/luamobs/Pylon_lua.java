...
public class Pylon_lua extends Pylon implements LuaMob {
...
@Override
    ...
    public void activate() {
        LuaValue luaScript = CustomObject.getScript(identifier);
        if (luaScript != null && !luaScript.get("activate").isnil()) {
            try {
                MethodOverride.VoidA0 superMethod = () -> super.activate();
               luaScript.get("activate").call(CoerceJavaToLua.coerce(this), vars, CoerceJavaToLua.coerce(superMethod));
            } catch (LuaError error) { Game.runOnRenderThread(()->	DungeonScene.show(new WndError(error))); }
        }
        super.activate();
    }
