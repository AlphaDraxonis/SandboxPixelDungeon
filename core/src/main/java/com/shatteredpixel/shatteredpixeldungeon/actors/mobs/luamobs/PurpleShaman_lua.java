...
public class PurpleShaman_lua extends Shaman.PurpleShaman implements LuaMob {
...
@Override
    ...
    protected void debuff(Char arg0) {
        ...
        if (luaScript != null && !luaScript.get("debuff").isnil()) {
            try {
                MethodOverride.VoidA1 superMethod = (a0) -> super.debuff((Char) a0);
               luaScript.get("debuff").invoke(new LuaValue[]{CoerceJavaToLua.coerce(this), vars, CoerceJavaToLua.coerce(superMethod), CoerceJavaToLua.coerce(arg0)}).arg1();
            } catch (LuaError error) { Game.runOnRenderThread(() -> DungeonScene.show(new WndError(error))); }
        }
        super.debuff(arg0);
    }
