...
public class FungalSpinner_lua extends FungalSpinner implements LuaMob {
...
@Override
    ...
    @Override
    protected void applyWebToCell(int arg0) {
        LuaValue luaScript = CustomObject.getScript(identifier);
        if (luaScript != null && !luaScript.get("applyWebToCell").isnil()) {
            try {
                MethodOverride.VoidA1 superMethod = (a0) -> super.applyWebToCell((int) a0);
               luaScript.get("applyWebToCell").invoke(new LuaValue[]{CoerceJavaToLua.coerce(this), vars, CoerceJavaToLua.coerce(superMethod), LuaValue.valueOf(arg0)}).arg1();
            } catch (LuaError error) { Game.runOnRenderThread(()->	DungeonScene.show(new WndError(error))); }
        }
        super.applyWebToCell(arg0);
    }
    ...
    public int webPos() {
        LuaValue luaScript = CustomObject.getScript(identifier);
        if (luaScript != null && !luaScript.get("webPos").isnil()) {
            try {
                MethodOverride.A0<Integer> superMethod = () -> super.webPos();
               int ret = luaScript.get("webPos").call(CoerceJavaToLua.coerce(this), vars, CoerceJavaToLua.coerce(superMethod)).toint();
                return ret;
            } catch (LuaError error) { Game.runOnRenderThread(()->	DungeonScene.show(new WndError(error))); }
        }
        return super.webPos();
    ...
    public void shootWeb() {
        LuaValue luaScript = CustomObject.getScript(identifier);
        if (luaScript != null && !luaScript.get("shootWeb").isnil()) {
            try {
                MethodOverride.VoidA0 superMethod = () -> super.shootWeb();
               luaScript.get("shootWeb").call(CoerceJavaToLua.coerce(this), vars, CoerceJavaToLua.coerce(superMethod));
            } catch (LuaError error) { Game.runOnRenderThread(()->	DungeonScene.show(new WndError(error))); }
        }
        super.shootWeb();
    }
