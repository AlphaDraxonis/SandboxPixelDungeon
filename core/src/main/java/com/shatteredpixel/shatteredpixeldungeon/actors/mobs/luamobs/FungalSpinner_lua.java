...
@Override
    ...
    protected void applyWebToCell(int arg0) {
        ...
        if (luaScript != null && !luaScript.get("applyWebToCell").isnil()) {
            try {
                luaScript.get("applyWebToCell").call(CoerceJavaToLua.coerce(this), vars, LuaValue.valueOf(arg0));
                return;
            } catch (LuaError error) { Game.runOnRenderThread(() -> DungeonScene.show(new WndError(error))); }
        }
    ...
    public void super_applyWebToCell(int arg0) {
        super.applyWebToCell(arg0);
    }
    ...
    public void shootWeb() {
        ...
        if (luaScript != null && !luaScript.get("shootWeb").isnil()) {
            try {
                luaScript.get("shootWeb").call(CoerceJavaToLua.coerce(this), vars);
                return;
            } catch (LuaError error) { Game.runOnRenderThread(() -> DungeonScene.show(new WndError(error))); }
        }
        super.shootWeb();
    ...
    public int webPos() {
        ...
        if (luaScript != null && !luaScript.get("webPos").isnil()) {
            try {
                int ret = luaScript.get("webPos").call(CoerceJavaToLua.coerce(this), vars).toint();
                return ret;
            } catch (LuaError error) { Game.runOnRenderThread(() -> DungeonScene.show(new WndError(error))); }
        }
        return super.webPos();
