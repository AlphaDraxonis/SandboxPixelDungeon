...
@Override
    ...
    public HashSet getSubjects() {
        ...
        if (luaScript != null && !luaScript.get("getSubjects").isnil()) {
            try {
                MethodOverride.A0<HashSet> superMethod = () -> super.getSubjects();
               HashSet ret = (HashSet) luaScript.get("getSubjects").call(CoerceJavaToLua.coerce(this), vars, CoerceJavaToLua.coerce(superMethod)).touserdata();
                return ret;
            } catch (LuaError error) { Game.runOnRenderThread(() -> DungeonScene.show(new WndError(error))); }
        }
