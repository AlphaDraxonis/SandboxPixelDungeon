...
public class DwarfKing_lua extends DwarfKing implements LuaMob {
...
@Override
    ...
    public HashSet getSubjects() {
        LuaValue luaScript = CustomObject.getScript(identifier);
        if (luaScript != null && !luaScript.get("getSubjects").isnil()) {
            try {
                HashSet ret = (HashSet) luaScript.get("getSubjects").call(CoerceJavaToLua.coerce(this), vars).touserdata();
                return ret;
            } catch (LuaError error) { Game.runOnRenderThread(() -> DungeonScene.show(new WndError(error))); }
        }
    ...
    public HashSet super_getSubjects() {
        return super.getSubjects();
    }
