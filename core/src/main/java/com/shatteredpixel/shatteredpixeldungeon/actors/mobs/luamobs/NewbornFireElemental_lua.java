...
public class NewbornFireElemental_lua extends Elemental.NewbornFireElemental implements LuaMob {
...
@Override
    ...
    public void setSummonedALly() {
        LuaValue luaScript = CustomObject.getScript(identifier);
        if (luaScript != null && !luaScript.get("setSummonedALly").isnil()) {
            try {
                MethodOverride.VoidA0 superMethod = () -> super.setSummonedALly();
               luaScript.get("setSummonedALly").call(CoerceJavaToLua.coerce(this), vars, CoerceJavaToLua.coerce(superMethod));
            } catch (LuaError error) { Game.runOnRenderThread(()->	DungeonScene.show(new WndError(error))); }
        }
        super.setSummonedALly();
    }
    ...
    @Override
    protected void rangedProc(Char arg0) {
        LuaValue luaScript = CustomObject.getScript(identifier);
        if (luaScript != null && !luaScript.get("rangedProc").isnil()) {
            try {
                MethodOverride.VoidA1 superMethod = (a0) -> super.rangedProc((Char) a0);
               luaScript.get("rangedProc").invoke(new LuaValue[]{CoerceJavaToLua.coerce(this), vars, CoerceJavaToLua.coerce(superMethod), CoerceJavaToLua.coerce(arg0)}).arg1();
            } catch (LuaError error) { Game.runOnRenderThread(()->	DungeonScene.show(new WndError(error))); }
        }
        super.rangedProc(arg0);
    }
    ...
    @Override
    protected void meleeProc(Char arg0, int arg1) {
        LuaValue luaScript = CustomObject.getScript(identifier);
        if (luaScript != null && !luaScript.get("meleeProc").isnil()) {
            try {
                MethodOverride.VoidA2 superMethod = (a0, a1) -> super.meleeProc((Char) a0, (int) a1);
               luaScript.get("meleeProc").invoke(new LuaValue[]{CoerceJavaToLua.coerce(this), vars, CoerceJavaToLua.coerce(superMethod), CoerceJavaToLua.coerce(arg0), LuaValue.valueOf(arg1)}).arg1();
            } catch (LuaError error) { Game.runOnRenderThread(()->	DungeonScene.show(new WndError(error))); }
        }
        super.meleeProc(arg0, arg1);
    }
