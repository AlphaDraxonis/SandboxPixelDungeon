...
public class ArmoredStatue_lua extends ArmoredStatue implements LuaMob {
...
@Override
    ...
    @Override
    public void createItems(boolean arg0) {
        LuaValue luaScript = CustomObject.getScript(identifier);
        if (luaScript != null && !luaScript.get("createItems").isnil()) {
            try {
                MethodOverride.VoidA1 superMethod = (a0) -> super.createItems((boolean) a0);
               luaScript.get("createItems").invoke(new LuaValue[]{CoerceJavaToLua.coerce(this), vars, CoerceJavaToLua.coerce(superMethod), LuaValue.valueOf(arg0)}).arg1();
            } catch (LuaError error) { Game.runOnRenderThread(() -> DungeonScene.show(new WndError(error))); }
        }
        super.createItems(arg0);
    }
    ...
    public ItemSelector.NullTypeSelector useNullWeapon() {
        LuaValue luaScript = CustomObject.getScript(identifier);
        if (luaScript != null && !luaScript.get("useNullWeapon").isnil()) {
            try {
                MethodOverride.A0<ItemSelector.NullTypeSelector> superMethod = () -> super.useNullWeapon();
               ItemSelector.NullTypeSelector ret = (ItemSelector.NullTypeSelector) luaScript.get("useNullWeapon").call(CoerceJavaToLua.coerce(this), vars, CoerceJavaToLua.coerce(superMethod)).touserdata();
                return ret;
            } catch (LuaError error) { Game.runOnRenderThread(() -> DungeonScene.show(new WndError(error))); }
        }
        return super.useNullWeapon();
    }
    ...
    public ItemSelector.NullTypeSelector useNullArmor() {
        LuaValue luaScript = CustomObject.getScript(identifier);
        if (luaScript != null && !luaScript.get("useNullArmor").isnil()) {
            try {
                MethodOverride.A0<ItemSelector.NullTypeSelector> superMethod = () -> super.useNullArmor();
               ItemSelector.NullTypeSelector ret = (ItemSelector.NullTypeSelector) luaScript.get("useNullArmor").call(CoerceJavaToLua.coerce(this), vars, CoerceJavaToLua.coerce(superMethod)).touserdata();
                return ret;
            } catch (LuaError error) { Game.runOnRenderThread(() -> DungeonScene.show(new WndError(error))); }
        }
        return super.useNullArmor();
    }
