...
public class Eye_lua extends Eye implements LuaMob {
...
@Override
    ...
    public void deathGaze() {
        LuaValue luaScript = CustomObject.getScript(identifier);
        if (luaScript != null && !luaScript.get("deathGaze").isnil()) {
            try {
                MethodOverride.VoidA0 superMethod = () -> super.deathGaze();
               luaScript.get("deathGaze").call(CoerceJavaToLua.coerce(this), vars, CoerceJavaToLua.coerce(superMethod));
            } catch (LuaError error) { Game.runOnRenderThread(()->	DungeonScene.show(new WndError(error))); }
        }
        super.deathGaze();
    }
