...
@Override
    ...
    public boolean canTele(int arg0) {
        ...
        if (luaScript != null && !luaScript.get("canTele").isnil()) {
            try {
                boolean ret = luaScript.get("canTele").call(CoerceJavaToLua.coerce(this), vars, LuaValue.valueOf(arg0)).toboolean();
                return ret;
            } catch (LuaError error) { Game.runOnRenderThread(() -> DungeonScene.show(new WndError(error))); }
        }
    ...
    public boolean super_canTele(int arg0) {
        return super.canTele(arg0);
    }
    ...
    public void teleportEnemy() {
        ...
        if (luaScript != null && !luaScript.get("teleportEnemy").isnil()) {
            try {
                luaScript.get("teleportEnemy").call(CoerceJavaToLua.coerce(this), vars);
                return;
            } catch (LuaError error) { Game.runOnRenderThread(() -> DungeonScene.show(new WndError(error))); }
        }
        super.teleportEnemy();
    ...
    public void super_teleportEnemy() {
        super.teleportEnemy();
    }
