...
public class Golem_lua extends Golem implements LuaMob {
...
@Override
    ...
    @Override
    public boolean canTele(int arg0) {
        LuaValue luaScript = CustomObject.getScript(identifier);
        if (luaScript != null && !luaScript.get("canTele").isnil()) {
            try {
                MethodOverride.A1<Boolean> superMethod = (a0) -> super.canTele((int) a0);
               boolean ret = luaScript.get("canTele").invoke(new LuaValue[]{CoerceJavaToLua.coerce(this), vars, CoerceJavaToLua.coerce(superMethod), LuaValue.valueOf(arg0)}).arg1().toboolean();
                return ret;
            } catch (LuaError error) { Game.runOnRenderThread(() -> DungeonScene.show(new WndError(error))); }
        }
        return super.canTele(arg0);
    }
    ...
    public void teleportEnemy() {
        LuaValue luaScript = CustomObject.getScript(identifier);
        if (luaScript != null && !luaScript.get("teleportEnemy").isnil()) {
            try {
                MethodOverride.VoidA0 superMethod = () -> super.teleportEnemy();
               luaScript.get("teleportEnemy").call(CoerceJavaToLua.coerce(this), vars, CoerceJavaToLua.coerce(superMethod));
            } catch (LuaError error) { Game.runOnRenderThread(() -> DungeonScene.show(new WndError(error))); }
        }
        super.teleportEnemy();
    }
