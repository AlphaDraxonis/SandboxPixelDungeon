...
public class Warlock_lua extends Warlock implements LuaMob {
...
@Override
    ...
    public void call() {
        LuaValue luaScript = CustomObject.getScript(identifier);
        if (luaScript != null && !luaScript.get("call").isnil()) {
            try {
                MethodOverride.VoidA0 superMethod = () -> super.call();
               luaScript.get("call").call(CoerceJavaToLua.coerce(this), vars, CoerceJavaToLua.coerce(superMethod));
            } catch (LuaError error) { Game.runOnRenderThread(() -> DungeonScene.show(new WndError(error))); }
        }
        super.call();
    }
