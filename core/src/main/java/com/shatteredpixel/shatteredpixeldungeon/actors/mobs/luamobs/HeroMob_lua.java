...
@Override
    ...
    public Weapon weapon() {
        ...
        if (luaScript != null && !luaScript.get("weapon").isnil()) {
            try {
                MethodOverride.A0<Weapon> superMethod = () -> super.weapon();
               Weapon ret = (Weapon) luaScript.get("weapon").call(CoerceJavaToLua.coerce(this), vars, CoerceJavaToLua.coerce(superMethod)).touserdata();
                return ret;
            } catch (LuaError error) { Game.runOnRenderThread(() -> DungeonScene.show(new WndError(error))); }
        }
        return super.weapon();
