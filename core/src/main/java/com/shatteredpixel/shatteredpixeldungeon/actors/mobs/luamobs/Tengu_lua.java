...
@Override
    ...
    public boolean useAbility() {
        ...
        if (luaScript != null && !luaScript.get("useAbility").isnil()) {
            try {
                MethodOverride.A0<Boolean> superMethod = () -> super.useAbility();
               boolean ret = luaScript.get("useAbility").call(CoerceJavaToLua.coerce(this), vars, CoerceJavaToLua.coerce(superMethod)).toboolean();
                return ret;
            } catch (LuaError error) { Game.runOnRenderThread(() -> DungeonScene.show(new WndError(error))); }
        }
    ...
    public boolean canUseAbility() {
        ...
        if (luaScript != null && !luaScript.get("canUseAbility").isnil()) {
            try {
                MethodOverride.A0<Boolean> superMethod = () -> super.canUseAbility();
               boolean ret = luaScript.get("canUseAbility").call(CoerceJavaToLua.coerce(this), vars, CoerceJavaToLua.coerce(superMethod)).toboolean();
                return ret;
            } catch (LuaError error) { Game.runOnRenderThread(() -> DungeonScene.show(new WndError(error))); }
        }
