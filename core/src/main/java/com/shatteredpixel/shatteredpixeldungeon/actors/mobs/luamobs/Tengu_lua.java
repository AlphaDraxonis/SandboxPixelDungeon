...
public class Tengu_lua extends Tengu implements LuaMob {
...
@Override
    ...
    public boolean useAbility() {
        LuaValue luaScript = CustomObject.getScript(identifier);
        if (luaScript != null && !luaScript.get("useAbility").isnil()) {
            try {
                boolean ret = luaScript.get("useAbility").call(CoerceJavaToLua.coerce(this), vars).toboolean();
                return ret;
            } catch (LuaError error) { Game.runOnRenderThread(() -> DungeonScene.show(new WndError(error))); }
        }
    ...
    public boolean super_useAbility() {
        return super.useAbility();
    }
    ...
    public boolean canUseAbility() {
        LuaValue luaScript = CustomObject.getScript(identifier);
        if (luaScript != null && !luaScript.get("canUseAbility").isnil()) {
            try {
                boolean ret = luaScript.get("canUseAbility").call(CoerceJavaToLua.coerce(this), vars).toboolean();
                return ret;
            } catch (LuaError error) { Game.runOnRenderThread(() -> DungeonScene.show(new WndError(error))); }
        }
    ...
    public boolean super_canUseAbility() {
        return super.canUseAbility();
    }
