...
public class ArmoredBrute_lua extends ArmoredBrute implements LuaMob {
...
@Override
    ...
    protected void triggerEnrage() {
        LuaValue luaScript = CustomObject.getScript(identifier);
        if (luaScript != null && !luaScript.get("triggerEnrage").isnil()) {
            try {
                MethodOverride.VoidA0 superMethod = () -> super.triggerEnrage();
               luaScript.get("triggerEnrage").call(CoerceJavaToLua.coerce(this), vars, CoerceJavaToLua.coerce(superMethod));
            } catch (LuaError error) { Game.runOnRenderThread(()->	DungeonScene.show(new WndError(error))); }
        }
        super.triggerEnrage();
    }
