...
public class CrystalGuardian_lua extends CrystalGuardian implements LuaMob {
...
@Override
    ...
    public boolean recovering() {
        LuaValue luaScript = CustomObject.getScript(identifier);
        if (luaScript != null && !luaScript.get("recovering").isnil()) {
            try {
                MethodOverride.A0<Boolean> superMethod = () -> super.recovering();
               boolean ret = luaScript.get("recovering").call(CoerceJavaToLua.coerce(this), vars, CoerceJavaToLua.coerce(superMethod)).toboolean();
                return ret;
            } catch (LuaError error) { Game.runOnRenderThread(()->	DungeonScene.show(new WndError(error))); }
        }
