...
public class Succubus_lua extends Succubus implements LuaMob {
...
@Override
    ...
    @Override
    public boolean blink(int arg0) {
        LuaValue luaScript = CustomObject.getScript(identifier);
        if (luaScript != null && !luaScript.get("blink").isnil()) {
            try {
                MethodOverride.A1<Boolean> superMethod = (a0) -> super.blink((int) a0);
               boolean ret = luaScript.get("blink").invoke(new LuaValue[]{CoerceJavaToLua.coerce(this), vars, CoerceJavaToLua.coerce(superMethod), LuaValue.valueOf(arg0)}).arg1().toboolean();
                return ret;
            } catch (LuaError error) { Game.runOnRenderThread(() -> DungeonScene.show(new WndError(error))); }
        }
        return super.blink(arg0);
    }
