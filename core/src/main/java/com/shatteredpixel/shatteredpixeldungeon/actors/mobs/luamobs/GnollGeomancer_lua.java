...
public class GnollGeomancer_lua extends GnollGeomancer implements LuaMob {
...
@Override
    ...
    public boolean hasSapper() {
        LuaValue luaScript = CustomObject.getScript(identifier);
        if (luaScript != null && !luaScript.get("hasSapper").isnil()) {
            try {
                MethodOverride.A0<Boolean> superMethod = () -> super.hasSapper();
               boolean ret = luaScript.get("hasSapper").call(CoerceJavaToLua.coerce(this), vars, CoerceJavaToLua.coerce(superMethod)).toboolean();
                return ret;
            } catch (LuaError error) { Game.runOnRenderThread(() -> DungeonScene.show(new WndError(error))); }
        }
        return super.hasSapper();
    }
    ...
    @Override
    public void linkSapper(GnollSapper arg0) {
        LuaValue luaScript = CustomObject.getScript(identifier);
        if (luaScript != null && !luaScript.get("linkSapper").isnil()) {
            try {
                MethodOverride.VoidA1 superMethod = (a0) -> super.linkSapper((GnollSapper) a0);
               luaScript.get("linkSapper").invoke(new LuaValue[]{CoerceJavaToLua.coerce(this), vars, CoerceJavaToLua.coerce(superMethod), CoerceJavaToLua.coerce(arg0)}).arg1();
            } catch (LuaError error) { Game.runOnRenderThread(() -> DungeonScene.show(new WndError(error))); }
        }
        super.linkSapper(arg0);
    }

    @Override
    public void loseSapper() {
        LuaValue luaScript = CustomObject.getScript(identifier);
        if (luaScript != null && !luaScript.get("loseSapper").isnil()) {
            try {
                MethodOverride.VoidA0 superMethod = () -> super.loseSapper();
               luaScript.get("loseSapper").call(CoerceJavaToLua.coerce(this), vars, CoerceJavaToLua.coerce(superMethod));
            } catch (LuaError error) { Game.runOnRenderThread(() -> DungeonScene.show(new WndError(error))); }
        }
        super.loseSapper();
    }
