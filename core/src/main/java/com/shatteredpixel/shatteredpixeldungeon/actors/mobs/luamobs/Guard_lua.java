...
public class Guard_lua extends Guard implements LuaMob {
...
@Override
    ...
    @Override
    public boolean chain(int arg0) {
        LuaValue luaScript = CustomObject.getScript(identifier);
        if (luaScript != null && !luaScript.get("chain").isnil()) {
            try {
                MethodOverride.A1<Boolean> superMethod = (a0) -> super.chain((int) a0);
               boolean ret = luaScript.get("chain").invoke(new LuaValue[]{CoerceJavaToLua.coerce(this), vars, CoerceJavaToLua.coerce(superMethod), LuaValue.valueOf(arg0)}).arg1().toboolean();
                return ret;
            } catch (LuaError error) { Game.runOnRenderThread(() -> DungeonScene.show(new WndError(error))); }
        }
