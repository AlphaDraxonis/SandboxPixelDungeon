...
public class BurningFist_lua extends YogFist.BurningFist implements LuaMob {
...
@Override
    ...
    protected void incrementRangedCooldown() {
        ...
        if (luaScript != null && !luaScript.get("incrementRangedCooldown").isnil()) {
            try {
                luaScript.get("incrementRangedCooldown").call(CoerceJavaToLua.coerce(this), vars);
                return;
            } catch (LuaError error) { Game.runOnRenderThread(() -> DungeonScene.show(new WndError(error))); }
        }
    ...
    public void super_incrementRangedCooldown() {
        super.incrementRangedCooldown();
    }
    ...
    protected boolean isNearYog() {
        ...
        if (luaScript != null && !luaScript.get("isNearYog").isnil()) {
            try {
                boolean ret = luaScript.get("isNearYog").call(CoerceJavaToLua.coerce(this), vars).toboolean();
                return ret;
            } catch (LuaError error) { Game.runOnRenderThread(() -> DungeonScene.show(new WndError(error))); }
        }
    ...
    public boolean super_isNearYog() {
        return super.isNearYog();
    }
