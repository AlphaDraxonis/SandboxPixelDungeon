...
public class DM200_lua extends DM200 implements LuaMob {
...
@Override
    ...
    @Override
    protected boolean canVent(int arg0) {
        LuaValue luaScript = CustomObject.getScript(identifier);
        if (luaScript != null && !luaScript.get("canVent").isnil()) {
            try {
                MethodOverride.A1<Boolean> superMethod = (a0) -> super.canVent((int) a0);
               boolean ret = luaScript.get("canVent").invoke(new LuaValue[]{CoerceJavaToLua.coerce(this), vars, CoerceJavaToLua.coerce(superMethod), LuaValue.valueOf(arg0)}).arg1().toboolean();
                return ret;
            } catch (LuaError error) { Game.runOnRenderThread(() -> DungeonScene.show(new WndError(error))); }
        }
        return super.canVent(arg0);
    }
