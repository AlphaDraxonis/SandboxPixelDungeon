...
public class GnollSapper_lua extends GnollSapper implements LuaMob {
...
@Override
    ...
    public Actor getPartner() {
        LuaValue luaScript = CustomObject.getScript(identifier);
        if (luaScript != null && !luaScript.get("getPartner").isnil()) {
            try {
                MethodOverride.A0<Actor> superMethod = () -> super.getPartner();
               Actor ret = (Actor) luaScript.get("getPartner").call(CoerceJavaToLua.coerce(this), vars, CoerceJavaToLua.coerce(superMethod)).touserdata();
                return ret;
            } catch (LuaError error) { Game.runOnRenderThread(()->	DungeonScene.show(new WndError(error))); }
        }
    ...
    public void losePartner() {
        LuaValue luaScript = CustomObject.getScript(identifier);
        if (luaScript != null && !luaScript.get("losePartner").isnil()) {
            try {
                MethodOverride.VoidA0 superMethod = () -> super.losePartner();
               luaScript.get("losePartner").call(CoerceJavaToLua.coerce(this), vars, CoerceJavaToLua.coerce(superMethod));
            } catch (LuaError error) { Game.runOnRenderThread(()->	DungeonScene.show(new WndError(error))); }
        }
        super.losePartner();
    }
    ...
    @Override
    public void linkPartner(Char arg0) {
        LuaValue luaScript = CustomObject.getScript(identifier);
        if (luaScript != null && !luaScript.get("linkPartner").isnil()) {
            try {
                MethodOverride.VoidA1 superMethod = (a0) -> super.linkPartner((Char) a0);
               luaScript.get("linkPartner").invoke(new LuaValue[]{CoerceJavaToLua.coerce(this), vars, CoerceJavaToLua.coerce(superMethod), CoerceJavaToLua.coerce(arg0)}).arg1();
            } catch (LuaError error) { Game.runOnRenderThread(()->	DungeonScene.show(new WndError(error))); }
        }
        super.linkPartner(arg0);
    }
