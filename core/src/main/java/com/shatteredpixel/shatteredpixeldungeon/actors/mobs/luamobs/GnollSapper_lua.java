...
@Override
    ...
    public Actor getPartner() {
        ...
        if (luaScript != null && !luaScript.get("getPartner").isnil()) {
            try {
                Actor ret = (Actor) luaScript.get("getPartner").call(CoerceJavaToLua.coerce(this), vars).touserdata();
                return ret;
            } catch (LuaError error) { Game.runOnRenderThread(() -> DungeonScene.show(new WndError(error))); }
        }
    ...
    public void losePartner() {
        ...
        if (luaScript != null && !luaScript.get("losePartner").isnil()) {
            try {
                luaScript.get("losePartner").call(CoerceJavaToLua.coerce(this), vars);
                return;
            } catch (LuaError error) { Game.runOnRenderThread(() -> DungeonScene.show(new WndError(error))); }
        }
        super.losePartner();
    ...
    public void linkPartner(Char arg0) {
        ...
        if (luaScript != null && !luaScript.get("linkPartner").isnil()) {
            try {
                luaScript.get("linkPartner").call(CoerceJavaToLua.coerce(this), vars, CoerceJavaToLua.coerce(arg0));
                return;
            } catch (LuaError error) { Game.runOnRenderThread(() -> DungeonScene.show(new WndError(error))); }
        }
    ...
    public void super_linkPartner(Char arg0) {
        super.linkPartner(arg0);
    }
