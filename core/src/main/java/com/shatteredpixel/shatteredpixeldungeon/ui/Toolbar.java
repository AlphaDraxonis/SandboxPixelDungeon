...
public class Toolbar extends Component {
	...
	public void update() {
		...
		if (lastEnabled && waitLastEnabled != (arrowCell == null || arrowCell.allowsWaiting(Dungeon.hero))) {
			waitLastEnabled = arrowCell == null || arrowCell.allowsWaiting(Dungeon.hero);
	...
	public static class Tool extends Button {
		...
		public void copyFromImage(Image other){//only use directly after instantiating
			RectF frame = base.frame();
			base.copy(other);
			base.frame(frame);
			layout();
		}
	...
	public static class QuickslotTool extends Tool {
		...
		protected int borderLeft = 2;
		protected int borderRight = 2;
	...
	public static class SlotSwapTool extends Tool {
		...
		public void updateVisuals(){
			...
			for (int i = 1; i < 4; i++){
				...
				if (items[i] != null){
					if (items[i] instanceof TileItem)
						icons[i] = new ItemSprite((TileItem) items[i]);
					else icons[i] = new ItemSprite(items[i]);
